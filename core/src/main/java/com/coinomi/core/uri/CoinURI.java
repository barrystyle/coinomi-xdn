...
/**
 * <p>Provides a standard implementation of a DigitalNote URI with support for the following:</p>
 *
 * <ul>
...
public class CoinURI implements Serializable {
    ...
    private void parseParameters(String[] nameValuePairTokens, List<CoinType> possibleTypes) throws CoinURIParseException {
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (sepIndex == -1)
                throw new CoinURIParseException("Malformed DigitalNote URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new CoinURIParseException("Malformed DigitalNote URI - empty name '" +
                        nameValuePairToken + "'");
    ...
    /**
     * The DigitalNote Address from the URI, if one was present. It's possible to have DigitalNote URI's with no address if a
